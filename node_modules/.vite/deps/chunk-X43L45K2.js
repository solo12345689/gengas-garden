...
function interpolate_default(a, b) {
  var x06 = a[0] * radians2, y06 = a[1] * radians2, x12 = b[0] * radians2, y12 = b[1] * radians2, cy0 = cos(y06), sy0 = sin(y06), cy1 = cos(y12), sy1 = sin(y12), kx0 = cy0 * cos(x06), ky0 = cy0 * sin(x06), kx1 = cy1 * cos(x12), ky1 = cy1 * sin(x12), d = 2 * asin(sqrt2(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k = sin(d);
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a = a0 + a1, b = b0 + b1, c3 = c0 + c1, m = sqrt2(a * a + b * b + c3 * c3), phi2 = asin(c3 /= m), lambda22 = abs(abs(c3) - 1) < epsilon || abs(lambda04 - lambda12) < epsilon ? (lambda04 + lambda12) / 2 : atan2(b, a), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
...
function conicConformalRaw(y06, y12) {
  var cy0 = cos(y06), n = y06 === y12 ? sin(y06) : log2(cy0 / cos(y12)) / log2(tany(y12) / tany(y06)), f = cy0 * pow2(tany(y06), n) / n;
  if (!n)
    return mercatorRaw;
  function project(x, y) {
    if (f > 0) {
  ...
  project.invert = function(x, y) {
    var fy = f - y, r = sign(n) * sqrt2(x * x + fy * fy), l = atan2(x, abs(fy)) * sign(fy);
    if (fy * n < 0)
      l -= pi * sign(x) * sign(fy);
...
function identity_default3() {
  var k = 1, tx = 0, ty = 0, sx = 1, sy = 1, alpha = 0, ca3, sa, x06 = null, y06, x12, y12, kx = 1, ky = 1, transform = transformer4({
    point: function(x, y) {
  ...
  _project(x06, y06, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c3 = (this.ymin - y06) / vy) < t)
        y = this.ymin, x = x06 + (t = c3) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax)
        return null;
      if ((c3 = (this.ymax - y06) / vy) < t)
        y = this.ymax, x = x06 + (t = c3) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax)
        return null;
      if ((c3 = (this.xmax - x06) / vx) < t)
        x = this.xmax, y = y06 + (t = c3) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin)
        return null;
      if ((c3 = (this.xmin - x06) / vx) < t)
        x = this.xmin, y = y06 + (t = c3) * vy;
    }
    return [x, y];
  }
  _edgecode(x, y) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i) {
  ...
  find(x, y, i = 0) {
    ...
    const i0 = i;
    let c3;
    while ((c3 = this._step(i, x, y)) >= 0 && c3 !== i && c3 !== i0)
      i = c3;
    return c3;
  }
  _step(i, x, y) {
...
//# sourceMappingURL=chunk-X43L45K2.js.map
